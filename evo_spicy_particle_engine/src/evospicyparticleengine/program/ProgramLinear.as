package evospicyparticleengine.program {
	 ...
	 * @author simo
	 */
	public final class ProgramLinear implements IProgram {
		
		private var ease:int;
		private var fromFullBright:Boolean;
		private var dimmDuringLife:Boolean;
		
		function ProgramLinear(ease:int = EaseProgram.LINEAR, fromFullBright:Boolean = false, dimmDuringLife:Boolean = false):void
		{
			this.ease = ease;
		...
		public final function get(context3d:Context3D):Program3D
		{
			...
			*/					
			...
			agalFragmentSource +=	"sub ft0, ft0, ft0.x \n";      			// prepare for alpha kill
			agalFragmentSource +=	"kil ft0.y \n"; 						// Alpha Kill red pixels
											
			agalFragmentSource +=	"mul ft0, v2, v2 \n";					// colors
